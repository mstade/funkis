...
function lazy(next, length) {
  ...
  var done = once(pipe(head, function(head) {
    return head === seq.done
  }))

  var first = once(pipe(done, function(done) {
    return done? null : head()
  }))

  var rest = once(pipe(done, function(done) {
    return done? null : lazy(next, length - 1)
  }))

  var size = once(pipe(done, function(done) {
    return done? 0 : length
  }))

  var iterate = (function(curr) {
    return pipe(done, function(empty) {
      if (empty) {
        return seq.done
